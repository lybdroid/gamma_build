...
export CROSS_COMPILE := $(ROOT_DIR)/gcc64/bin/aarch64-elf-
...
export KCFLAGS += -Wno-int-in-bool-context
export KCFLAGS += -Wno-sometimes-uninitialized
export KCFLAGS += -Wno-tautological-constant-out-of-range-compare
export KCFLAGS += -Wno-duplicate-decl-specifier
export KCFLAGS += -Wno-address-of-packed-member
export KCFLAGS += -Wno-format-invalid-specifier
export KCFLAGS += -Wno-gnu
export KCFLAGS += -Wno-void-pointer-to-enum-cast
export KCFLAGS += -fno-builtin
############################################################################
########################### KERNEL BUILD STEPS #############################
############################################################################
